...
public class BacnetObject extends BacnetProperty {
    ...
    static final String ACTION_WRITE = "write@priority";
    ...
    BacnetObject(BacnetDevice device, Node node, ObjectIdentifier oid) {
        ...
        this.hiddenStateTextProp = new HiddenProperty(device, this, oid,
                PropertyIdentifier.stateText);
        this.hiddenActiveTextProp = new HiddenProperty(device, this, oid,
                PropertyIdentifier.activeText);
        this.hiddenInactiveTextProp = new HiddenProperty(device, this, oid,
                PropertyIdentifier.inactiveText);
        this.hiddenActionTextProp = new HiddenProperty(device, this, oid,
    ...
    private void addProperty(PropertyIdentifier propid, Node child) {
        ...
        if (child == null) {
            child = node.createChild(name, true).setValueType(ValueType.STRING)
    ...
    private SequenceOf<PropertyIdentifier> getPropertyList() {
        return (SequenceOf<PropertyIdentifier>) Utils
                .readProperty(device.conn, device, oid, PropertyIdentifier.propertyList, null);
    }

    private void initHistory() {
        if (Utils.isOneOf(oid.getObjectType(), ObjectType.trendLog, ObjectType.trendLogMultiple,
                ObjectType.eventLog)) {
    ...
    private void makeWriteAction() {
        if (isWritable()) {
            Action act = new Action(Permission.WRITE, event -> handleWrite(event));
            act.addParameter(new Parameter("Value", node.getValueType(), node.getValue()));
            act.addParameter(new Parameter("Priority", ValueType.NUMBER, new Value(writePriority)));
            act.addParameter(new Parameter("Relinquish", ValueType.BOOL, new Value(false)));
            Node anode = node.getChild(ACTION_WRITE, false);
            if (anode == null) {
                node.createChild(ACTION_WRITE, false).setAction(act).build().setSerializable(false);
            } else {
                anode.setAction(act);
            }
        }
    ...
    private void handleWrite(ActionResult event) {
        Value value = event.getParameter("Value");
        int priority = event.getParameter("Priority", ValueType.NUMBER).getNumber().intValue();
        if (event.getParameter("Relinquish", ValueType.BOOL).getBool()) {
            value = null;
        }
    ...
    protected boolean shouldBeSettable() {
        return Utils.isOneOf(oid.getObjectType(), ObjectType.analogOutput, ObjectType.analogValue,
                ...
                ObjectType.positiveIntegerValue, ObjectType.dateValue,
                ObjectType.datetimePatternValue,
                ObjectType.datePatternValue, ObjectType.timePatternValue, ObjectType.channel,
    ...
    private void write(Value newval, int priority) {
        ...
        if (newval != null) {
            ...
            switch (type) {
                case BINARY: {
                    enc = (newval.getBool() != null && newval.getBool()) ? BinaryPV.active :
                            BinaryPV.inactive;
                    break;
                }
                case MULTISTATE: {
        ...
        ServiceFuture sf = sendWriteRequest(
    ...
    private void makeDiscoverAction() {
        ...
        if (anode == null) {
            node.createChild(ACTION_DISCOVER_PROPERTIES, true).setAction(act).build()
    ...
    private void makeAddPropertyAction() {
        ...
        act.addParameter(
                new Parameter("Property Identifier", ValueType.makeEnum(Utils.getPropertyList())));
        Node anode = node.getChild(ACTION_ADD_PROPERTY, true);
        if (anode == null) {
            node.createChild(ACTION_ADD_PROPERTY, true).setAction(act).build()
                    .setSerializable(false);
        } else {
            anode.setAction(act);
        }
    ...
    protected void makeEditAction() {
        ...
        act.addParameter(new Parameter("Enable Headless Polling", ValueType.BOOL,
                new Value(headlessPolling)));
        act.addParameter(
                new Parameter("Write Priority", ValueType.NUMBER, new Value(writePriority)));
        act.addParameter(
                new Parameter(CONFIG_WRITABLE, ValueType.BOOL, node.getRoConfig(CONFIG_WRITABLE)));
    ...
    private void edit(ActionResult event) {
        ...
        int newPriority = event.getParameter("Write Priority", ValueType.NUMBER).getNumber()
                .intValue();
            ...
            if (newCovUse) {
                for (BacnetProperty property : properties) {
                    if (PropertyIdentifier.presentValue.equals(pid)
                        ...
                        device.unsubscribeProperty(property);
    ...
    protected void subscribe() {
        ...
        if (Utils.isOneOf(type, ObjectType.binaryInput, ObjectType.binaryOutput,
                ObjectType.binaryValue)) {
        ...
        switch (type) {
            case BINARY: {
                ...
                if (value instanceof com.serotonin.bacnet4j.type.primitive.Boolean) {
                    v = new Value(
                            ((com.serotonin.bacnet4j.type.primitive.Boolean) value).booleanValue());
...
//		}
    ...
    public void updateProperty(Encodable value, PropertyIdentifier propid) {
        ...
        if (PropertyIdentifier.objectName.equals(propid)) {
            ...
            node.getLink().getSubscriptionManager().postChildUpdate(node, false);
                ...
                PropertyIdentifier.actionText.equals(propid)) {
            @SuppressWarnings("unchecked")
    ...
    public void covNotificationReceived(UnsignedInteger timeRemaining,
    ...
    private static int findPropertyInSequence(SequenceOf<PropertyValue> seq,
    ...
    private class Db extends Database {
        ...
        public void query(String path, long from, long to, CompleteHandler<QueryData> handler) {
            Encodable enc = Utils
                    .readProperty(device.conn, device, oid, PropertyIdentifier.recordCount, null);
            ...
            ReadRangeRequest request = new ReadRangeRequest(oid, PropertyIdentifier.logBuffer, null,
                    byTime);
            while (request != null) {
        ...
        private ReadRangeRequest processReadRangeAck(long to, int recordCount, ReadRangeAck ack,
            ...
            if (ack.getResultFlags().isMoreItems()) {
                ...
                BySequenceNumber bySeq = new BySequenceNumber(
                        ack.getFirstSequenceNumber().longValue() + itemCount,
        ...
        private long getTimestampFromRecord(Encodable record) {
            if (record instanceof LogRecord) {
                ...
                try {
                    DateTime dt = (DateTime) record.getClass().getMethod("getTimestamp")
                            .invoke(record);
