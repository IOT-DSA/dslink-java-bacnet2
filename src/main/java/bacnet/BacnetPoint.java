...
public class BacnetPoint {
	...
	public void setObjectTypeId(int objectTypeId) {
		...
		// else
		// node.createChild("objectTypeId").setValueType(ValueType.NUMBER).setValue(new
		// Value(objectTypeId)).build();
	...
	public void setInstanceNumber(int instanceNumber) {
		...
		// else
		// node.createChild("instanceNumber").setValueType(ValueType.NUMBER).setValue(new
		// Value(instanceNumber)).build();
	...
	public void setObjectTypeDescription(String objectTypeDescription) {
		...
		// else
		// node.createChild("objectTypeDescription").setValueType(ValueType.STRING).setValue(new
		// Value(objectTypeDescription)).build();
		// System.out.println("objectTypeDescription updated to " +
	...
	public void setObjectName(String objectName) {
		...
		if (node != null) {
			...
			// else
			// node.createChild("objectName").setValueType(ValueType.STRING).setValue(new
			// Value(objectName)).build();
	...
	private class GetLogHandler implements Handler<ActionResult> {
		...
		public void handle(ActionResult event) {
			...
			if (choice == 0) {
				...
				Date d = new Date(Integer.parseInt(dtstr.substring(0, 4)),
						Month.valueOf(Integer.parseInt(dtstr.substring(5, 7))),
						Integer.parseInt(dtstr.substring(8, 10)), null);
				Time t = new Time(Integer.parseInt(dtstr.substring(11, 13)), Integer.parseInt(dtstr.substring(14, 16)),
						Integer.parseInt(dtstr.substring(17, 19)), Integer.parseInt(dtstr.substring(20, 22)));
	...
	private void handleSet(Value newval, int priority, boolean raw) {
		...
		if (dataType == DataType.BINARY) {
			if (raw) {
				// newval = String.valueOf(Boolean.parseBoolean(newval) ||
				// newval.equals("1"));
			} else {
	...
	public void setDataType(DataType dataType) {
		...
		// else
		// node.createChild("dataType").setValueType(ValueType.STRING).setValue(new
		// Value(dataType.toString())).build();
	...
	public void setUnitsDescription(List<String> unitsDescription) {
		...
		// if (vnode != null) vnode.setValue(new
		// Value(unitsDescription.toString()));
		// else
		// node.createChild("unitsDescription").setValueType(ValueType.STRING).setValue(new
		// Value(unitsDescription.toString())).build();
		// System.out.println("unitsDescription updated to " +
		// unitsDescription);
		// }
	}
	...
	public void setPresentValue(String presentValue, PropertyIdentifier pid) {
		...
		// else vnode = node.createChild("present
		// value").setValueType(ValueType.STRING).setValue(new
		// Value(presentValue)).build();
		...
		// act.addParameter(new Parameter("value", ValueType.STRING, new
		// Value(presentValue)));
	...
	public void setCov(boolean cov) {
		...
		// else
		// node.createChild("cov").setValueType(ValueType.BOOL).setValue(new
		// Value(cov)).build();
	...
	public void setEngineeringUnits(String engineeringUnits) {
		...
		// else
		// node.createChild("engineeringUnits").setValueType(ValueType.STRING).setValue(new
		// Value(engineeringUnits)).build();
		// System.out.println("engineeringUnits updated to " +
		// engineeringUnits);
		// }
	}
	...
	public void setReferenceDevice(String referenceDevice) {
		...
		// else
		// node.createChild("referenceDeviceId").setValueType(ValueType.NUMBER).setValue(new
		// Value(referenceDeviceId)).build();
		// System.out.println("referenceDeviceId updated to " +
		// referenceDeviceId);
		// }
	}
	...
	public void setReferenceObject(String referenceObject) {
		...
		// if (vnode != null) vnode.setValue(new
		// Value(referenceObjectTypeDescription));
		// else
		// node.createChild("referenceObjectTypeDescription").setValueType(ValueType.STRING).setValue(new
		// Value(referenceObjectTypeDescription)).build();
		// System.out.println("referenceObjectTypeDescription updated to " +
		// referenceObjectTypeDescription);
		// }
	}
	...
	void update() {
		...
		if (presentValue != null) {
			// String prettyVal = getPrettyPresentValue(objectTypeId,
			// presentValue, unitsDescription, referenceObjectTypeDescription,
			// referenceInstanceNumber, referenceDeviceId);
			ValueType vt;
			Value val;
			switch (dataType) {
			case BINARY: {
	...
	private void makeSetAction(Node valnode, int priority) {
		...
		// switch(dataType) {
		// case BINARY: {
		// String off = (unitsDescription.size() > 0) ? unitsDescription.get(0)
		// : "0";
		// String on = (unitsDescription.size() > 1) ? unitsDescription.get(1) :
		// "1";
		// String def = ("0".equals(presentValue)) ? off : on;
		// par = new Parameter("value", ValueType.makeEnum(on, off), new
		// Value(def));
		// break;
		// }
		// case NUMERIC: {
		// par = new Parameter("value", ValueType.STRING, new
		// Value(presentValue));
		// break;
		// }
		// case MULTISTATE: {
		...
		// int index = Integer.parseInt(presentValue) - 1;
		// if (index >= 0 && index < unitsDescription.size())
		// par = new Parameter("value", ValueType.makeEnum(enums), new
		// Value(unitsDescription.get(index)));
		// } catch (NumberFormatException e) {
		// // no op
		// }
		// break;
